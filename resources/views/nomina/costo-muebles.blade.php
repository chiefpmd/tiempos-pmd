...
function recalcularTotales() {
    ...
    document.querySelectorAll('tbody tr[data-mueble-id]').forEach(row => {
        const input = row.querySelector('.costo-input');
        const valorMueble = input ? (parseFloat(input.value) || 0) : 0;
        const totalText = row.querySelector('.total-nomina').textContent;
        const totalNomina = parseFloat(totalText.replace(/[$,]/g, '')) || 0;
    ...
    // Valor Mueble total (penúltima columna numérica)
    const valorCell = footerCells[footerCells.length - 2];
    ...
    if (granValor > 0) {
        const pct = (granTotal / granValor * 100);
        pctCell.textContent = pct.toFixed(1) + '%';
